...
export const GET_BY_ID = api.GET(async (ctx) => {
  const prompt = await promptService.getPrompt(ctx.params.id, ctx.user.id);
...
export const PATCH = api.PATCH(async (ctx, body) => {
  ...
  const okResult = await promptService.updatePrompt(ctx.params.id, ctx.user.id, parsed.data);
...
export const DELETE = api.DELETE(async (ctx) => {
  const okResult = await promptService.deletePrompt(ctx.params.id, ctx.user.id);
  if (!okResult) return notFound();
  return ok({ ok: true });
});
...
export const POST_RATING = api.POST(async (ctx) => {
  ...
  await promptService.createRating(ctx.params.id, ctx.user.id, parsed.data);
  return ok({ ok: true });
});
