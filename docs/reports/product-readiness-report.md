...
| 6 | **Billing** | **7/10** | Good | Dodo Payments checkout/portal/webhook wired, webhook now reachable (was blocked — fixed), credits system with plan math, product envs. Gaps: no end-to-end test event verified, no refunds/plan-change flows verified, subscription lifecycle edge cases (trialing→canceled) untested. |
...
4. **Billing e2e (7/10 → 8.5/10)** — run `scripts/dodo-sandbox/checkout-sandbox.cjs` (sandbox test-card checkout through `/api/webhooks/dodo`) and verify the subscription flips to active.
