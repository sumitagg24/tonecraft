...
| 0.2 | **Unblock Dodo webhook** — `/api/webhooks/dodo` is behind the auth proxy, so paid subscriptions never activate | `src/proxy.ts` PUBLIC_PATHS vs the Dodo webhook route (audit 06 H1) | Whitelist `/api/webhooks/dodo` (Dodo signature check is the auth); verify end-to-end with a test event |
