...
| **Dodo Payments billing** | ✅ | Signature verification (`Webhooks() Standard Webhooks verification` + `DODO_PAYMENTS_WEBHOOK_KEY`); environment from DODO_PAYMENTS_ENVIRONMENT; ownership-bound checkout (`customData.userId` from session, never body); entitlement sync with plan caching + invalidation; monthly + annual price IDs; past_due grace. |
...
| **Secrets** | ✅ | `.env*` gitignored; `.env.example` contains **no** real values; no `NEXT_PUBLIC_*` secret keys (only publishable Clerk key, Dodo client token, APP_URL, Sentry DSN, socket URL). |
...
| 8 | P2 webhook replay | Dodo/Svix redelivery re-processed events and re-wrote audit logs | `WebhookEvent` table + `claimWebhookEvent`/`markWebhookProcessed` in both webhook routes (replays return early; crashed attempts retry). |
